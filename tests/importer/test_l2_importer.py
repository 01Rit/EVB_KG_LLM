...
def test_create_cross_layer_relations():
    ...
    mock_neo4j.execute_query.side_effect = [
        [{'cnt': 1}],  # USES_TOOL (component+tool pair matches)
    ...
    assert relations == 7  # USES_TOOL(1) + ORIGINATED_FROM(3) + REFERENCED_IN(3) = 7

    calls = mock_neo4j.execute_query.call_args_list
    assert len(calls) == 3

    uses_tool_cypher = calls[0][0][0]
    assert 'USES_TOOL' in uses_tool_cypher

    originated_from_cypher = calls[1][0][0]
    assert 'ORIGINATED_FROM' in originated_from_cypher

    referenced_in_cypher = calls[2][0][0]
