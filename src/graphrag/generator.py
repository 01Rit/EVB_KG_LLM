...
【并行拆卸规则】
如果两个零件之间没有直接依赖关系（不在对方的 depends_on 中），且它们的前置依赖都已满足，则它们可以并行拆卸。

判断逻辑：
- 如果零件A的 depends_on 包含零件B，或者零件B的 depends_on 包含零件A，则不能并行
- 如果零件A和零件B没有任何直接或传递的依赖关系，且它们的前置依赖都已完成，则可以并行

示例：
- BMC -> 冷却板，BMC -> 冷却管
- 冷却板 和 冷却管 可以并行拆卸（因为它们都只依赖BMC，且BMC已完成）
